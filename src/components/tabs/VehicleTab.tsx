...
export function VehicleTab(p: Props) {
  return (
    <motion.div key="veicolo" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="space-y-6 pb-24">
      ...
      {p.selectedCar ? (
        <div className="bg-[#0a0f1d] p-9 rounded-[48px] border border-blue-500/30 space-y-10 relative overflow-hidden shadow-2xl">
          <div className="absolute top-0 right-0 w-80 h-80 bg-blue-600/10 blur-[100px] pointer-events-none rounded-full" />
          <div className="flex justify-between items-start relative z-10">
            <div>
              <div className="text-[11px] font-black uppercase tracking-[0.4em] text-blue-400 mb-3">Veicolo Attivo</div>
              <h3 className="text-4xl font-black uppercase italic tracking-tighter text-white leading-tight">{p.selectedCar.model}</h3>
              <div className="text-[11px] font-bold text-[#8e8e93] uppercase tracking-widest opacity-60 mt-2">{p.selectedCar.tags}</div>
            </div>
            <button 
              onClick={() => p.setSelectedCar(null)} 
              className="px-5 py-2.5 bg-white/5 hover:bg-red-500/10 rounded-full text-[10px] font-black uppercase tracking-widest text-[#8e8e93] hover:text-red-400 transition-all border border-white/10"
            >
              Rimuovi
            </button>
          </div>
          <div className="grid grid-cols-2 gap-5 relative z-10">
            <div className="bg-black/40 backdrop-blur-xl p-7 rounded-[32px] border border-white/5 shadow-inner">
              <div className="text-[10px] font-black text-[#8e8e93] uppercase tracking-[0.2em] mb-2">Serbatoio</div>
              <div className="text-4xl font-black italic text-white tracking-tighter">{p.selectedCar.liters}<span className="text-sm ml-1 text-blue-500 font-black">L</span></div>
            </div>
            <div className="bg-black/40 backdrop-blur-xl p-7 rounded-[32px] border border-white/5 shadow-inner">
              <div className="text-[10px] font-black text-[#8e8e93] uppercase tracking-[0.2em] mb-2">Consumo</div>
              <div className="text-4xl font-black italic text-white tracking-tighter">{p.selectedCar.kml}<span className="text-sm ml-1 text-blue-500 font-black">KM/L</span></div>
            </div>
          </div>
          <div className="pt-8 border-t border-white/10 relative z-10">
            <div className="flex justify-between items-center p-7 bg-blue-600/10 rounded-[40px] border border-blue-500/20 shadow-2xl backdrop-blur-md">
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 bg-blue-600 text-white rounded-full flex items-center justify-center shadow-2xl border border-blue-400/30">
                  <Zap size={26} className="fill-white" />
                </div>
                <div>
                  <div className="text-[11px] font-black uppercase tracking-[0.3em] text-blue-400 mb-0.5">Efficienza</div>
                  <div className="text-[13px] font-black text-white uppercase italic tracking-tight">Range Ottimale</div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-3xl font-black italic text-white tracking-tighter">{p.selectedCar.kml}</div>
