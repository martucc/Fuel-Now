...
import { Search, SlidersHorizontal, Crown, Navigation, TrendingDown, TrendingUp, RefreshCw, Zap, Clock, Activity } from 'lucide-react';
...
export function HomeTab(p: Props) {
  ...
  // === VERDETTO IA HOME: solo 3 esiti netti ===
  const verdict = useMemo(() => {
    const fc = p.marketRef?.forecast || [];
    if (fc.length < 2) return null;
    const fStart = fc[0].price;
    const fEnd = fc[fc.length - 1].price;
    if (!fStart || !fEnd) return null;
    const fcDelta = ((fEnd - fStart) / fStart) * 100;
    const savings = (fStart - fEnd) * p.tankLiters;
    let v: { label: string; action: string; tone: string; bg: string; border: string; icon: 'fill' | 'wait' | 'minimal' };
    if (fcDelta < -0.3) {
      // Prezzo scende → aspetta
      v = { label: 'Aspetta', action: `prezzo in calo del ${Math.abs(fcDelta).toFixed(1)}% in 7 giorni`, tone: 'text-emerald-400', bg: 'bg-emerald-500/12', border: 'border-emerald-500/30', icon: 'wait' };
    } else if (fcDelta > 0.3) {
      // Prezzo sale → fai pieno ora
      v = { label: 'Fai il pieno', action: `prezzo in salita del ${fcDelta.toFixed(1)}% in 7 giorni`, tone: 'text-red-400', bg: 'bg-red-500/12', border: 'border-red-500/30', icon: 'fill' };
    } else {
      // Mercato laterale → metti il minimo
      v = { label: 'Pochi euro', action: 'mercato stabile, metti solo lo stretto necessario', tone: 'text-amber-400', bg: 'bg-amber-500/12', border: 'border-amber-500/30', icon: 'minimal' };
    }
    return { ...v, fcDelta, savings };
  ...
  return (
    <motion.div key="home" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="space-y-6 pb-32">
            ...
            <div className="relative z-10">
              <div className="flex items-start justify-between gap-3 mb-10">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-black flex items-center justify-center shadow-[0_0_30px_rgba(37,99,235,0.4)] overflow-hidden border border-white/10 z-10 flex-shrink-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <h2 className="text-lg sm:text-xl font-black text-white tracking-tighter leading-tight uppercase italic truncate">{bestStation.city || bestStation.name}</h2>
                    <p className="text-[11px] text-[#8e8e93] font-bold mt-0.5 truncate">{bestStation.brand} &bull; {bestStation.distance || '1.2'} km</p>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 px-2.5 sm:px-3.5 py-1.5 rounded-full border border-blue-500 bg-blue-500/10 flex-shrink-0">
                  <Crown className="w-3.5 h-3.5 text-blue-400" />
                  <span className="text-[10px] sm:text-[11px] font-black text-blue-400 uppercase tracking-widest">Alpha</span>
                </div>
              </div>
              
              <div className="flex items-end justify-between gap-3 mb-8 px-1">
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-black text-[#8e8e93] uppercase tracking-widest mb-1">Prezzo/Litro</p>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-[40px] sm:text-[48px] font-black text-[#4ade80] tabular-nums tracking-tighter leading-none">
                      {p.cheapestPrice.toFixed(3)}
                    </span>
                    <span className="text-base sm:text-lg font-black text-white/30">EUR</span>
                  </div>
                </div>
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="text-[26px] sm:text-[32px] font-black text-white tabular-nums tracking-tighter">
      ...
      {/* Intelligence Widget */}
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-[#09090b]/50 backdrop-blur-xl border border-white/5 rounded-[32px] sm:rounded-[40px] p-5 sm:p-7 shadow-2xl">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            {verdict && (
              <div className={cn("flex items-center gap-1.5 text-[11px] font-black px-3 py-1.5 rounded-full border tabular-nums",
                verdict.fcDelta < -0.3 ? 'text-[#4ade80] bg-[#4ade80]/10 border-[#4ade80]/20'
                : verdict.fcDelta > 0.3 ? 'text-red-400 bg-red-500/10 border-red-500/20'
                : 'text-amber-400 bg-amber-500/10 border-amber-500/20'
              )}>
                {verdict.fcDelta < -0.3 ? <TrendingDown className="w-3.5 h-3.5" />
                  : verdict.fcDelta > 0.3 ? <TrendingUp className="w-3.5 h-3.5" />
                  : <Activity className="w-3.5 h-3.5" />}
                {verdict.fcDelta > 0 ? '+' : ''}{verdict.fcDelta.toFixed(1)}%
              </div>
            )}
            <button
        ...
        {verdict && (
          <div className={cn("rounded-2xl border p-4 mb-6", verdict.bg, verdict.border)}>
            <div className="flex items-center gap-3 mb-1">
              {verdict.icon === 'fill' ? <Zap className={cn("w-5 h-5", verdict.tone)} />
                : verdict.icon === 'wait' ? <Clock className={cn("w-5 h-5", verdict.tone)} />
                : <Activity className={cn("w-5 h-5", verdict.tone)} />}
              <h3 className={cn("text-[24px] sm:text-[28px] font-black tracking-tight leading-none italic uppercase", verdict.tone)}>
                {verdict.label}
              </h3>
            </div>
            <p className="text-[12px] text-zinc-300 ml-8 font-medium">{verdict.action}</p>
            {Math.abs(verdict.savings) >= 0.5 && (
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/5 ml-8">
                <span className="text-[10px] text-zinc-500 uppercase tracking-wider font-semibold">
                  {verdict.savings > 0 ? `Risparmi se aspetti (${p.tankLiters}L)` : `Perdi se aspetti (${p.tankLiters}L)`}
                </span>
                <span className={cn("text-[15px] font-black tabular-nums", verdict.savings > 0 ? 'text-emerald-400' : 'text-red-400')}>
                  {verdict.savings > 0 ? '+' : '-'}€{Math.abs(verdict.savings).toFixed(2)}
                </span>
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4 items-center mb-8">
          <div className="text-center space-y-2 min-w-0">
            <p className="text-[9px] sm:text-[10px] font-bold text-[#8e8e93] uppercase tracking-widest">Media</p>
            <p className="text-xl sm:text-2xl font-black text-white tabular-nums tracking-tight">{p.averagePrice !== Infinity ? p.averagePrice.toFixed(3) : '-'}</p>
          </div>
          <div className="text-center py-5 px-1 sm:px-2 bg-emerald-500/5 rounded-[24px] sm:rounded-[32px] border border-emerald-500/20 shadow-inner min-w-0">
            <p className="text-[9px] sm:text-[10px] font-black text-[#4ade80] uppercase tracking-widest mb-1">Più Economico</p>
            <p className="text-xl sm:text-2xl font-black text-[#4ade80] tabular-nums tracking-tight">{p.cheapestPrice !== Infinity ? p.cheapestPrice.toFixed(3) : '-'}</p>
          </div>
          <div className="text-center space-y-2 min-w-0">
            <p className="text-[9px] sm:text-[10px] font-bold text-[#8e8e93] uppercase tracking-widest">Spread</p>
            <p className="text-xl sm:text-2xl font-black text-white tabular-nums tracking-tight">{spread > 0 ? spread.toFixed(3) : '-'}</p>
          </div>
        </div>
            ...
            <div className="bg-white/5 rounded-[32px] p-6 border border-white/5 relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between mb-4 relative z-10">
                <div className="flex items-center gap-2">
                  <div className={cn("w-1.5 h-1.5 rounded-full", verdict?.icon === 'fill' ? "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,1)]" : verdict?.icon === 'wait' ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,1)]" : "bg-amber-500")} />
                ...
                {verdict && (
                  <div className={cn("px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border",
                    verdict.icon === 'fill' ? "text-red-400 border-red-500/30 bg-red-500/10" :
                    verdict.icon === 'wait' ? "text-emerald-400 border-emerald-500/30 bg-emerald-500/10" :
                    "text-amber-400 border-amber-500/30 bg-amber-500/10"
                  )}>
                    {verdict.icon === 'fill' ? 'Pieno Consigliato' :
                     verdict.icon === 'wait' ? 'Attendi Calo' :
                     'Rifornimento Minimo'}
                  </div>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/5 rounded-[28px] p-5 border border-white/5 flex flex-col items-center justify-center space-y-1">
                ...
                <div className="flex items-center gap-1.5">
                  {verdict ? (
                    <>
                      {verdict.fcDelta < -0.3 ? <TrendingDown className="w-3 h-3 text-emerald-400" />
                        : verdict.fcDelta > 0.3 ? <TrendingUp className="w-3 h-3 text-red-400" />
                        : <Activity className="w-3 h-3 text-amber-400" />}
                      <p className={cn("text-lg font-black tracking-tighter tabular-nums",
                        verdict.fcDelta < -0.3 ? 'text-emerald-400'
                        : verdict.fcDelta > 0.3 ? 'text-red-400'
                        : 'text-amber-400'
                      )}>
                        {verdict.fcDelta > 0 ? '+' : ''}{verdict.fcDelta.toFixed(1)}%
                      </p>
                    ...
                    <p className="text-lg font-black text-white/40 tracking-tighter tabular-nums">—</p>
                  )}
                </div>
              </div>
              <div className="bg-white/5 rounded-[28px] p-5 border border-white/5 flex flex-col items-center justify-center space-y-1">
                <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.3em]">Trend Atteso</p>
                <p className={cn("text-lg font-black tracking-tighter uppercase italic",
                  !verdict ? 'text-white/40'
                  : verdict.fcDelta < -0.3 ? 'text-emerald-400'
                  : verdict.fcDelta > 0.3 ? 'text-red-400'
                  : 'text-amber-400'
                )}>
                  {!verdict ? '—'
                    : verdict.fcDelta < -0.3 ? 'Ribasso'
                    : verdict.fcDelta > 0.3 ? 'Rialzo'
                    : 'Stabile'}
                </p>
              </div>
            </div>
      ...
      >
        <SavingsWidget fuel={p.selectedFuel} cheapestLocal={p.cheapestPrice} tankLiters={p.tankLiters} />
      </motion.div>
